...
class Ml_100K():
    __BASE = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'Data/last.fm')
...
def readRecData(path,test_ratio=0.2):
    ...
    #test_set = random.sample(triples,1024)
    train_set = list(set(triples)-set(test_set))
    #print('len', len(triples))
