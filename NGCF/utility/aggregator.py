...
class Aggregator(torch.nn.Module):  # 单层邻居聚合方式
    '''
    Aggregator class
    ...
    def forward(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings, act):
        ...
        if batch_size != self.batch_size:
            self.batch_size = batch_size
        neighbors_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, user_embeddings)  # 得到该实体的邻居表征

        if self.aggregator == 'sum':
            output = (self_vectors + neighbors_agg).view((-1, self.dim))

        elif self.aggregator == 'concat':  # 然后聚合自身
        ...
        return act(output.view((self.batch_size, -1, self.dim)))  # 得到实体聚合邻居后的表征  其实就是单层的邻居聚合·方式
